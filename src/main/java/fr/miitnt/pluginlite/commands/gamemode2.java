...
public class gamemode2 implements CommandExecutor {
    ...
    public gamemode2(Pluginlite pluginlite) {
    ...
    public boolean onCommand(CommandSender sender, final Command cmd, final String msg, final String[] args) {
        if (sender instanceof Player) {
            ...
            if (p.isOp() || p.hasPermission("pluginlite.gamemode.adventure")) {
                if (args.length == 0) {
                    p.sendMessage(Objects.requireNonNull(main.getConfig().getString("ServerName")) + " " + Objects.requireNonNull(main.getConfig().getString("messages.gamemode2")));
